...
| POST | `/api/invoices/awards/sync` | 打開發票頁時自動跑一輪。號碼過期才去抓,補對獎與通知每次都做 |
| POST | `/api/invoices/awards/refresh` | 手動重抓號碼並補對獎。號碼一定去抓,失敗回 502,`detail` 帶原始錯誤 |
...
- 中獎號碼來自財政部稅務入口網的公開 RSS(`invoice.etax.nat.gov.tw/invoice.xml`),
  ...
  卻會讓人誤以為自己沒中。
- 這份 RSS 只公布特別獎、特獎、頭獎三組,這樣就夠了:二獎到六獎全部是從頭獎號碼
  由後往前逐碼放寬推出來的(末 7 碼 4 萬、末 6 碼 1 萬、末 5 碼 4000、末 4 碼 1000、
  **末 3 碼 200**),不是另外公布的號碼。
- 「增開六獎」是另一回事,它是額外公布的三位數號碼。自 111 年 1-2 月期起已停辦,
  改為增開雲端發票專屬獎(500 元、65 萬組),所以 RSS 沒有這一欄是正確的,不是缺口。
  `extra_sixth` 欄位與手動輸入仍保留,萬一日後恢復不必再改結構。
- 雲端發票專屬獎碰不到,因為它只開給存在載具裡的雲端發票,而那種發票不會印出紙本
  證明聯,也就沒有 QR 可掃。這跟載具 API 拿不到 appID 是同一道牆。
- `tests/fixtures/invoice-awards-real.xml` 是這個來源的真實回應,原封不動存著。
  會留它是因為解析器曾經對真實資料**整份解不出來,而手寫的 fixture 全部照樣綠**:
  真實標題是 `<![CDATA[ 115年 03~04月 ]]>`,整串裡一個 `>` 都沒有,去 HTML 標籤的
  `<[^>]*>` 就從頭吃到尾把標題整個刪掉,期別認不出來便整期跳過。改動解析器時,
  要壞就讓它在那份 fixture 上壞。
- 對獎有兩個入口,走的是同一支 `runAwardCycle()`(號碼 → 補對獎 → 通知),差別只有
  「要不要強制抓號碼」與「寄誰的信」。Cron 每小時掃全站;使用者打開發票頁時也跑
  一次,只寄他自己的。**光靠 Cron 不夠**——使用者的認知是「點進去就會幫我對好」,
  只讀資料庫的話,看到的會是上一輪排程留下的舊狀態。號碼超過 6 小時才會真的去
  財政部要,所以進出頁面幾次不會一直打人家的站。
